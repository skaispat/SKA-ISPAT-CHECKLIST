    ...
    const fetchTasks = async () => {
        try {
            ...
            const { data: deptData, error: deptError } = await supabase
                .from('departments')
                .select('dept_name')
                .ilike('dept_name', 'SECURITY')
                .single()

            if (deptError) throw deptError

            if (!deptData) {
                console.error("Security department not found")
                setLoading(false)
                return
            }
        ...
        <AdminLayout>
            <div className="space-y-4 animate-in fade-in duration-500">
                ...
                <div className="flex flex-col sm:flex-row gap-6 justify-between items-start sm:items-center pb-6">
                    <div>
                        <h1 className="text-3xl font-semibold tracking-tight text-foreground">Security Records</h1>
                    </div>
...
function EditTaskInfoModal({ tasks, onClose, onUpdate, onRefresh }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const department = tasks[0]?.department || 'SECURITY'
    ...
    const tasksToDelete = selectedTitle
        ? tasks.filter(t => t.task_title === selectedTitle && t.department === (tasks[0]?.department || 'SECURITY'))
        : []
